...
def timestamp_text(value: str) -> str:
    ...
    for part in re.split(r"(\r\n|\r|\n)", value):
        if part in {"\r\n", "\r", "\n"}:
            ...
            if at_line_start:
                result.append(timestamp_prefix())
            result.append(part)
            at_line_start = False
    return "".join(result)
...
def parse_arguments() -> argparse.Namespace:
    parser = TimestampArgumentParser(
...
class CopyProgress:
    ...
    def update(self, file_index: int, copied_bytes: int, relative: str, force: bool = False) -> None:
        ...
        try:
            ...
            maximum = max(columns - TIMESTAMP_PREFIX_WIDTH - 8, 1)
...
def wait_for_health(package: PackageInfo, codex_bin_root: Path, timeout_seconds: int) -> ValidationResult:
    ...
    while True:
        ...
        if maximum is not None:
            line = fit_console_text(line, maximum)
        line_width = console_text_width(line)
        output_width = max(width, line_width)
        if maximum is not None:
            output_width = min(output_width, maximum)
        print("\r" + line + " " * max(output_width - line_width, 0), end="", flush=True)
        width = line_width
        if previous & current:
            print()
