...
$script:ExecutionTimer = [Diagnostics.Stopwatch]::StartNew()
...
function Add-TimestampToText {
    param([AllowEmptyString()][string]$Text)
    if ([string]::IsNullOrEmpty($Text)) { return $Text }
    $builder = [Text.StringBuilder]::new()
    $atLineStart = $true
    foreach ($character in $Text.ToCharArray()) {
        if ($atLineStart -and $character -ne "`r" -and $character -ne "`n") {
            [void]$builder.Append((Get-TimestampPrefix))
        }
        [void]$builder.Append($character)
        $atLineStart = $character -eq "`r" -or $character -eq "`n"
    }
...
function Write-Host {
    ...
    if ($null -eq $Object -or $Object.Count -eq 0) {
        Microsoft.PowerShell.Utility\Write-Host -NoNewline:$NoNewline
        return
    }
...
function Format-Elapsed {
    ...
    [long]$totalMilliseconds = [Math]::Max(0, [Math]::Round($TotalSeconds * 1000))
    [long]$totalWholeSeconds = [Math]::Floor($totalMilliseconds / 1000)
    [long]$milliseconds = $totalMilliseconds % 1000
    [long]$totalMinutes = [Math]::Floor($totalWholeSeconds / 60)
    [long]$seconds = $totalWholeSeconds % 60
    [long]$hours = [Math]::Floor($totalMinutes / 60)
...
function Show-CopyProgress {
    ...
    try {
        ...
        $maximum = [Math]::Max([Console]::BufferWidth - $script:TimestampPrefixWidth - 8, 1)
        ...
        [Console]::Write((Add-TimestampToText ("`r" + $line.PadRight($width))))
...
function Wait-CodexHealth {
    ...
    while ($true) {
        ...
        try { $maximum = [Math]::Max([Console]::BufferWidth - $script:TimestampPrefixWidth - 8, 1) } catch { $maximum = $null }
        if ($null -ne $maximum -and $line.Length -gt $maximum) { $line = if ($maximum -eq 1) { '.' } else { $line.Substring(0, $maximum - 1) + '…' } }
        $outputWidth = [Math]::Max($width, $line.Length); if ($null -ne $maximum) { $outputWidth = [Math]::Min($outputWidth, $maximum) }
        Write-Host ("`r" + $line + (' ' * [Math]::Max($outputWidth - $line.Length, 0))) -NoNewline; $width = $line.Length
...
try { $exitCode = Invoke-Main }
    ...
    $exitCode = if ($_.Exception.Data.Contains('RepairExitCode')) { [int]$_.Exception.Data['RepairExitCode'] } else { $script:EXIT_UNEXPECTED }
